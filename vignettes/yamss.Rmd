...
```{r}
dr <- diffrep(cmsobj, classes = classes)
head(dr)
```
...
dqs <- densityQuantiles(cmsobj)
dqs
cmsobj3 <- updatePeaks(cmsobj, dqs["98.5%"])
...
The raw spectral data from the experiment is available in a `cms` object in a few formats. 
