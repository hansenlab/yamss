...
The `r Biocpkg("yamss")` package provides tools to preprocess raw mass spectral data files arising from metabolomics experiments with the primary goal of providing high-quality differential analysis. Existing software packages such as XCMS (available on Bioconductor) and MZMine 2 also provide preprocessing functionality but focus more on feature detection and alignment in individual samples ([@xcms], [@mzmine]). We emphasize that `r Biocpkg("yamss")` implements many of the same stages as these packages (namely background correction, retention time alignment, and peak detection), but tht output of `r Biocpkg("yamss")` is targeted to more statistically powerful inference about comparisons between groups of samples.
...
```{r}
pheno <- data.frame(sampClasses = classes, ionmode = "pos")
cmsRawObj <- readMSdata(files = files, pheno = pheno, verbose = TRUE)
```

```{r}
cmsProcObj <- bakedpi(cmsRaw = cmsRawObj, dbandwidth = c(0.01, 10), dgridstep = c(0.01, 1),
...
```{r}
dr <- diffrep(cmsProcObj, classes = classes)
head(dr)
```
...
```{r}
dmat <- densityEstimate(cmsProcObj)
```
...
plotDensityRegion(cmsProcObj, mzrange = mzrange + c(-0.5,0.5), scanrange = scanrange + c(-30,30))
```
...
```{r}
cmsobj2 <- updatePeaks(cmsProcObj, densityCutoff(cmsProcObj)*0.99)
dqs <- densityQuantiles(cmsProcObj)
head(dqs)
cmsobj3 <- updatePeaks(cmsProcObj, dqs["98.5%"])
nrow(peakBounds(cmsobj)) # Number of peaks detected - original
nrow(peakBounds(cmsobj2)) # Number of peaks detected - updated
nrow(peakBounds(cmsobj3)) # Number of peaks detected - updated
```
