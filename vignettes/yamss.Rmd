...
Next step is to use `bakedpi()` to preprocess these samples.  This function takes a while to run, so we only run it on a small slice of MZ values, using the `mzsubset` argument.  This is only done for speed.

```{r}
cmsProc <- bakedpi(cmsRaw = cmsRaw, dbandwidth = c(0.01, 10), dgridstep = c(0.01, 1),
...
The resulting object is an instance of class `CMSproc` and contains several pieces of information used in preprocessing as well as differential analysis results.
...
```{r}
dr <- diffrep(cmsProc, classes = classes)
head(dr)
```
...
In addition to differential analysis results, `CMSproc` objects contain information useful in exploring your data.
...
```{r}
cmsProc2 <- updatePeaks(cmsProc, densityCutoff(cmsProc)*0.99)
dqs <- densityQuantiles(cmsProc)
head(dqs)
cmsProc3 <- updatePeaks(cmsProc, dqs["98.5%"])
nrow(peakBounds(cmsProc)) # Number of peaks detected - original
nrow(peakBounds(cmsProc2)) # Number of peaks detected - updated
nrow(peakBounds(cmsProc3)) # Number of peaks detected - updated
```
