...
getEICS <- function(cmsRaw, mzranges) {
    ...
    maxScan <- max(rawPeakDT[,scan])
    numSamples <- length(unique(rawPeakDT[,sample]))
    eics <- lapply(1:length(mzranges), function(i) {
        x <- rawPeakDT[.(seq(start(mzranges[i]), end(mzranges[i]))), nomatch = 0]
        if (nrow(x)==0)
